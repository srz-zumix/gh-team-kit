...
func NewCheckCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if submodules {
				...
				_ = renderer.RenderPermissions(repoPermissions) // nolint:errcheck
			} else {
				...
				_ = renderer.RenderPermission(permission) // nolint:errcheck
			}
	...
	f := cmd.Flags()
	f.BoolVar(&exitCode, "exit-code", false, "Return an exit code of 1 if the user has no permissions")
